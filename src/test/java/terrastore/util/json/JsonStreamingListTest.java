...
public class JsonStreamingListTest {
    ...
    public void testEmptyArray() {
        JsonStreamingList list = new JsonStreamingList(new Value(EMPTY.getBytes()));
    ...
    public void testTypes() {
        ...
        list = new JsonStreamingList(new Value(STRING.getBytes()));
        assertEquals(1, list.size());
        assertEquals("string", list.get(0));

        list = new JsonStreamingList(new Value(LONG.getBytes()));
        assertEquals(1, list.size());
        assertEquals(1L, list.get(0));

        list = new JsonStreamingList(new Value(FLOAT.getBytes()));
        assertEquals(1, list.size());
        assertEquals(1.5f, list.get(0));

        list = new JsonStreamingList(new Value(TRUE.getBytes()));
        assertEquals(1, list.size());
        assertEquals(true, list.get(0));

        list = new JsonStreamingList(new Value(FALSE.getBytes()));
        assertEquals(1, list.size());
        assertEquals(false, list.get(0));

        list = new JsonStreamingList(new Value(NULL.getBytes()));
        assertEquals(1, list.size());
        assertEquals(null, list.get(0));

        list = new JsonStreamingList(new Value(OBJECT.getBytes()));
        assertEquals(1, list.size());
        assertTrue(JsonStreamingMap.class.isAssignableFrom(list.get(0).getClass()));

        list = new JsonStreamingList(new Value(ARRAY.getBytes()));
        assertEquals(1, list.size());
    ...
    public void testCompositeArray() {
        JsonStreamingList list = new JsonStreamingList(new Value(COMPOSITE.getBytes()));
    ...
    public void testWithInnerObject() {
        JsonStreamingList list = new JsonStreamingList(new Value(INNER_OBJECT.getBytes()));
    ...
    public void testWithInnerArray() {
        JsonStreamingList list = new JsonStreamingList(new Value(INNER_ARRAY.getBytes()));
