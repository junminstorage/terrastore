...
public class DefaultDiscoveryTest {
    ...
    @Test
    public void testJoinAndBootstrapLater() throws Exception {
        Cluster cluster = new Cluster("cluster", false);
        ...
        seed.send(EasyMock.<MembershipCommand>anyObject());
        expectLastCall().andThrow(new ProcessingException(new ErrorMessage(0, "")));
        seed.send(EasyMock.<MembershipCommand>anyObject());
        expectLastCall().andReturn(new View("cluster", Sets.newHashSet(new View.Member("discovered", "localhost", 6000)))).once();
        seed.disconnect();
        expectLastCall().times(2);
        //
        Node discoveredNode = createMock(Node.class);
    ...
    @Test
    public void testScheduleRequiresToJoinFirst() throws Exception {
        RemoteNodeFactory nodeFactory = createMock(RemoteNodeFactory.class);
        makeThreadSafe(nodeFactory, true);
        //
        Router router = createMock(Router.class);
